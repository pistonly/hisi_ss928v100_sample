...
#define OT_SAMPLE_IVE_MD_PIC_WIDTH 352
#define OT_SAMPLE_IVE_MD_PIC_HEIGHT 288
#define OT_SAMPLE_IVE_MD_TOTAL_FRAME 700
...
static ot_sample_ivs_md_info g_md_file_info;
...
static td_void sample_ivs_md_uninit(ot_sample_ivs_md_info *md_info_ptr) {
  ...
  sample_svp_check_exps_return_void(md_info_ptr == TD_NULL,
                                    SAMPLE_SVP_ERR_LEVEL_ERROR,
                                    "md_inf_ptr can't be null\n");

  for (i = 0; i < OT_SAMPLE_IVE_MD_IMAGE_NUM; i++) {
    sample_svp_mmz_free(md_info_ptr->img[i].phys_addr[0],
                        md_info_ptr->img[i].virt_addr[0]);
  }
...
static td_s32 sample_ivs_md_file_init(ot_sample_ivs_md_info *md_inf_ptr,
  ...
  sample_svp_check_exps_return(md_inf_ptr == TD_NULL, ret,
                               SAMPLE_SVP_ERR_LEVEL_ERROR,
                               "md_inf_ptr can't be null\n");

  for (i = 0; i < OT_SAMPLE_IVE_MD_IMAGE_NUM; i++) {
    ret = sample_common_ive_create_image(&md_inf_ptr->img[i],
                                         OT_SVP_IMG_TYPE_U8C1, width, height);
    sample_svp_check_exps_goto(
        ret != TD_SUCCESS, md_init_fail, SAMPLE_SVP_ERR_LEVEL_ERROR,
        "Error(%#x),Create img[%d] image failed!\n", ret, i);
  }
  size = sizeof(ot_ive_ccblob);
  ret = sample_common_ive_create_mem_info(&md_inf_ptr->blob, size);
  sample_svp_check_exps_goto(ret != TD_SUCCESS, md_init_fail,
                             SAMPLE_SVP_ERR_LEVEL_ERROR,
...
static td_void sample_ivs_set_src_dst_size(ot_sample_src_dst_size *src_dst,
                                           td_u32 src_width, td_u32 src_height,
                                           td_u32 dst_width,
                                           td_u32 dst_height) {
...
static td_void *sample_ivs_md_file_proc(td_void *args) {
  td_s32 ret;
  ot_sample_ivs_md_info *md_ptr = (ot_sample_ivs_md_info *)(args);
  ot_video_frame_info
      frm[OT_SAMPLE_IVE_MD_VPSS_CHN]; /* 0:base_frm, 1:ext_frm */
  ot_sample_md_vo_vpss_hld hld = {0};
  td_s32 vpss_chn[] = {OT_VPSS_CHN0, OT_VPSS_CHN1};
  td_s32 cur_idx = 0;
  td_bool is_first_frm = TD_TRUE;

  sample_svp_check_exps_return(md_ptr == TD_NULL, TD_NULL,
                               SAMPLE_SVP_ERR_LEVEL_ERROR,
                               "md_inf_ptr can't be null\n");

  /* Create chn */
  ret = ot_ivs_md_create_chn(hld.md_chn, &(md_ptr->md_attr));
  sample_svp_check_exps_return(ret != TD_SUCCESS, TD_NULL,
                               SAMPLE_SVP_ERR_LEVEL_ERROR,
                               "ot_ivs_md_create_chn fail\n");
  ...
  while (g_stop_signal == TD_FALSE) {
    /* ret = ss_mpi_vpss_get_chn_frame(hld.vpss_grp, vpss_chn[1], &frm[1], */
    /*                                 OT_SAMPLE_IVE_MD_MILLIC_SEC); */
    /* sample_svp_check_exps_continue( */
    ...
    /* ret = ss_mpi_vpss_get_chn_frame(hld.vpss_grp, vpss_chn[0], &frm[0], */
    /*                                 OT_SAMPLE_IVE_MD_MILLIC_SEC); */
    /* sample_svp_check_failed_goto( */
    /*     ret, ext_free, SAMPLE_SVP_ERR_LEVEL_ERROR, */
    ...
    ret = sample_common_ive_blob_to_rect(
        sample_svp_convert_addr_to_ptr(ot_ive_ccblob, md_ptr->blob.virt_addr),
  ...
  sample_svp_check_failed_trace(ret, SAMPLE_SVP_ERR_LEVEL_ERROR,
                                "ot_ivs_md_destroy_chn fail,Err(%#x)\n", ret);
...
static td_s32 sample_ive_md_pause(td_void) {
  ...
  if (g_stop_signal == TD_TRUE) {
    ...
    sample_ivs_md_uninit(&(g_md_file_info));
    (td_void) memset_s(&g_md_file_info, sizeof(g_md_file_info), 0, sizeof(g_md_file_info));
...
td_void sample_ive_md_file(td_void) {
  ...
  ret = ot_ivs_md_init();
  sample_svp_check_exps_return(ret != TD_SUCCESS, ret, SAMPLE_SVP_ERROR_LEVEL_ERROR, "ot_ivs_md_init failed!\n", ret);

  /* ret = sample_ivs_md_file_init(&g_md_file_info, OT_SAMPLE_IVE_MD_PIC_WIDTH, */
  /*                          OT_SAMPLE_IVE_MD_PIC_HEIGHT); */
  /* sample_svp_check_exps_goto(ret != TD_SUCCESS, end_md_0, */
  /*                            SAMPLE_SVP_ERR_LEVEL_ERROR, */
  /*                            " Error(%#x),sample_ivs_md_file_init failed!\n", ret); */
  /* g_stop_signal = TD_FALSE; */
/*   /\* */
/*    * step 3: Create work thread */
/*    *\/ */
/*   ret = prctl(PR_SET_NAME, "ive_md_proc_file", 0, 0, 0); */
/*   sample_svp_check_exps_goto(ret != TD_SUCCESS, end_md_0, */
/*                              SAMPLE_SVP_ERR_LEVEL_ERROR, */
/*                              "set thread name failed!\n"); */
/*   ret = pthread_create(&g_md_thread, 0, sample_ivs_md_file_proc, */
/*                        (td_void *)&g_md_file_info); */
/*   sample_svp_check_exps_goto(ret != TD_SUCCESS, end_md_0, */
/*                              SAMPLE_SVP_ERR_LEVEL_ERROR, */
 ...
 */
td_void sample_ive_md_file_handle_sig(td_void)
{
    g_stop_signal = TD_TRUE;
}
