...
static td_void sample_vdec_cmd_ctrl(td_u32 chn_num,
                                    vdec_thread_param *vdec_send,
                                    pthread_t *vdec_thread, td_u32 send_arr_len,
                                    td_u32 thread_arr_len) {
  td_u32 i;
  ot_vdec_chn_status status;
  td_s32 c;

  for (i = 0; (i < chn_num) && (i < send_arr_len) && (i < thread_arr_len);
       i++) {
  ...
  sample_comm_vdec_cmd_not_circle_send(chn_num, vdec_send, vdec_thread,
                                       send_arr_len, thread_arr_len);
  return;
...
static td_void sample_send_stream_to_vdec(sample_vdec_attr *sample_vdec,
  ...
  for (i = 0; (i < vdec_chn_num) && (i < arr_len); i++) {
    if (snprintf_s(vdec_send[i].c_file_name, sizeof(vdec_send[i].c_file_name),
                   sizeof(vdec_send[i].c_file_name) - 1, stream_name) < 0) {
      return;
    }
    if (snprintf_s(vdec_send[i].c_file_path, sizeof(vdec_send[i].c_file_path),
                   sizeof(vdec_send[i].c_file_path) - 1, "%s",
                   SAMPLE_STREAM_PATH) < 0) {
      return;
    }
    ...
    vdec_send[i].min_buf_size =
        (sample_vdec[i].width * sample_vdec[i].height * 3) >> 1; /* 3:yuv */
    vdec_send[i].fps = 30; /* 30:frame rate */
  }
  sample_comm_vdec_start_send_stream(vdec_chn_num, &vdec_send[0],
                                     &vdec_thread[0], OT_VDEC_MAX_CHN_NUM,
                                     OT_VDEC_MAX_CHN_NUM);
  ...
  for (i = 0; i < 500; ++i) {
    ...
    ret = ss_mpi_vpss_get_chn_frame(vpss_grp, 1, &frame, -1);
    if (ret != TD_SUCCESS) {
  ...
  sample_comm_vdec_stop_send_stream(vdec_chn_num, &vdec_send[0],
                                    &vdec_thread[0], OT_VDEC_MAX_CHN_NUM,
                                    OT_VDEC_MAX_CHN_NUM);
}
...
static td_s32 sample_h265_vdec_vpss_vo(td_void) {
  ...
  *************************************************/
  ret = sample_init_sys_and_vb(&sample_vdec[0], vdec_chn_num, g_cur_type,
                               OT_VDEC_MAX_CHN_NUM);
  if (ret != TD_SUCCESS) {
    return ret;
  }
...
static td_s32 sample_check_parameter(int argc, const char *argv0,
                                     const char *argv1) {
