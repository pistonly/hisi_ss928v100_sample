...
static td_void sample_ive_usage(const td_char *name) {
  ...
  printf("index:\n");
  printf("\t 0)Canny,<complete>:0, part canny;1,complete "
         "canny.(FILE->IVE->FILE).\n");
...
static td_s32 sample_ive_case_with_two_arg(char *argv[]) {
  ...
  switch (*argv[1]) {
  case '0': {
    if (((*argv[OT_SAMPLE_IVE_ARG_IDX_TWO] != '0') &&
         (*argv[OT_SAMPLE_IVE_ARG_IDX_TWO] != '1')) ||
        (len != 1)) {
      return TD_FAILURE;
    }
...
#endif
{
  ...
  struct sigaction sa;
#endif
  if ((argc < OT_SAMPLE_IVE_ARG_NUM_TWO) ||
      (argc > OT_SAMPLE_IVE_ARG_NUM_THREE)) {
